...
exports.scrollerButton = exports.buyButton = undefined;
...
exports.createCounter = exports.setButtonVisibility = exports.changeProductAmount = undefined;
...
var productNodes = (0, _jquery2.default)('.card');

var changeProductAmount = function changeProductAmount(event, id) {
	var target = (0, _jquery2.default)(event.target);

	if (target.hasClass('counter__control--plus')) {
		_reducers.store.dispatch({ type: 'INCREASE_PRODUCT', id: id });
	} else if (target.hasClass('counter__control--minus') && getCurrentProduct(id).amount > 0) {
		_reducers.store.dispatch({ type: 'DECREASE_PRODUCT', id: id });
	};
};

var setButtonVisibility = function setButtonVisibility() {
	if (isAnyProductSelected()) {
		_reducers.store.dispatch({ type: 'SET_VISIBILITY', visibility: 'VISIBLE' });
	} else {
		_reducers.store.dispatch({ type: 'SET_VISIBILITY', visibility: 'HIDDEN' });
	};
};

var isAnyProductSelected = function isAnyProductSelected() {
	var amount = _reducers.store.getState().products.reduce(function (result, product) {
...
var getCurrentProduct = function getCurrentProduct(id) {
	return _reducers.store.getState().products.filter(function (product) {
		return product.id === id;
	})[0];
};

var handleAppearance = function handleAppearance(state, node) {
	if (state.amount === 0) {
...
var changeValue = function changeValue(node, value) {
	node.html(value);
};
...
(0, _jquery2.default)('.slider--pricelist .slider__inner').on('click', '.counter', function (event) {
	var card = (0, _jquery2.default)(event.target).closest('.card');
	var id = Number(card.data('id'));
...
var removeItem = function removeItem(event, id) {
	event.preventDefault();
	_reducers.store.dispatch({ type: 'ZEROIZE_PRODUCT', id: id });

	(0, _card.setButtonVisibility)();
};

var createArticle = function createArticle(product, parent) {
	...
	var price = (0, _jquery2.default)('<p/>').addClass('card__price').text(product.price * product.amount + ' \u0440\u0443\u0431.');
	var priceContainer = (0, _jquery2.default)('<div/>');
	var counter = (0, _card.createCounter)(product.amount);
	...
	counter.on('click', function (event) {
		event.preventDefault();
		(0, _card.changeProductAmount)(event, product.id);
		(0, _card.setButtonVisibility)();
	});

	closeButton.on('click', function (event) {
		return removeItem(event, product.id);
	});
};
...
var renderProducts = function renderProducts(products) {
	...
	var containers = (0, _jquery2.default)('.cart__inner');
	containers.html('');
	products.forEach(function (product) {
		containers.each(function (index, container) {
			return createArticle(product, (0, _jquery2.default)(container));
		});
	});
};

var showSum = function showSum(products) {
	var sum = products.reduce(function (result, product) {
		return result = result + product.amount * product.price;
	}, 0);

	var formSum = (0, _jquery2.default)('.form__sum-value');
	var cartSum = (0, _jquery2.default)('.cart__sum-value');
...
var item = function item(state, _ref2) {
	var type = _ref2.type,
	    title = _ref2.title,
	    id = _ref2.id,
	    price = _ref2.price,
...
$(function () {
	...
	var getAnchors = function getAnchors(selector) {
		var anchors = selector.find('.navbar__item');
		var visibleAnchors = anchors.filter(':visible');
		var ids = [].concat(_toConsumableArray(visibleAnchors)).map(function (anchor) {
			return anchor.dataset.helper;
		});

		visibleAnchors.each(function (index, node) {
